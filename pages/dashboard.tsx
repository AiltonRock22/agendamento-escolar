...
import type { PerfilProfessor, Recurso, Reserva } from '@/types/agendamento'
...
export default function Dashboard() {
  ...
  const [form, setForm] = useState({ recurso: 'video' as Recurso, data: dataLocal(1), horario: horarios[0], turma: '', finalidade: '' })
  ...
  async function salvar(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await criarReserva({ usuarioId: user.uid, nomeProfessor: user.displayName || user.email?.split('@')[0] || 'Professor', emailProfessor: user.email || '', perfilProfessor: perfil, ...form })
